...
# Layout e Configuração
# ==========================
st.set_page_config(page_title="BicoCont - Contagem Manual", layout="wide", page_icon="📦")
...
st.title("📦 BicoCont - Contagem Manual")
st.write("Sistema de contagem física e saldo SAP informados manualmente com análise visual de diferenças.")
...
with col1:
    ...
    if not matches.empty:
        selected_code = st.selectbox("Código", sorted(matches['code'].unique()))
        dep_options = matches[matches['code'] == selected_code][['deposit', 'name']].drop_duplicates().reset_index(drop=True)
        dep_map = dep_options.to_dict(orient='records')

        labels = [f"{d['deposit']}" for d in dep_map]
        sel_idx = st.selectbox("Depósito", options=list(range(len(labels))), format_func=lambda i: labels[i])
        selected_deposit = dep_map[sel_idx]['deposit']
        selected_name = dep_map[sel_idx]['name']
        ...
        sap_manual = st.number_input("Saldo SAP (informado manualmente)", min_value=0, value=0, step=1)
        physical = st.number_input("Contagem física", min_value=0, value=0, step=1)
        user = st.text_input("Usuário (opcional)", "")

        if st.button("Salvar contagem"):
            diff, ts = save_count(selected_code, selected_name, selected_deposit, sap_manual, physical, user)
            color = "🟡" if diff > 0 else ("🔴" if diff < 0 else "⚪")
            st.success(f"{color} Contagem salva. Diferença: {diff} (registrado em {ts})")
    else:
        st.warning("Nenhum material encontrado. Verifique o código ou nome digitado.")
    ...
    if uploaded:
        try:
            ...
            if st.button("Atualizar Base"):
                ...
                conn.commit()
                for _, row in df_new.iterrows():
                    cur.execute("INSERT INTO materials (code, name, deposit, sap) VALUES (?, ?, ?, 0)",
                                (row['code'], row['name'], row['deposit']))
                conn.commit()
                conn.close()
                st.success("Base atualizada com sucesso.")
                st.experimental_rerun()
...
if hist.empty:
    ...
    def color_diff(val):
        color = ''
        if val > 0:
            color = 'background-color: #fff3cd;'  # amarelo claro (sobra)
        elif val < 0:
            color = 'background-color: #f8d7da;'  # vermelho claro (falta)
        return color

    st.dataframe(hist.style.applymap(color_diff, subset=['diff']))
